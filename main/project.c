...
#define EXAMPLE_ESP_WIFI_SSID          "qtfreet"
#define EXAMPLE_ESP_WIFI_PASS          "melodysoyani"
#define MAX_RETRY                      10
...
static void set_direction(){
    ...
    gpio_set_direction(RAIN, GPIO_MODE_INPUT);
}
...
void Publisher_Task(void *params)
{
    char temp[10];
    char humi[10];
    char rain[20]= "khong mua";
    char rain1[20]= "co mua";
    int rain3 = 0;
    char rain4[2];
    int flag = 0;
    char kq[22] = {}; 
    ...
    while (true)
    {
        ...
        if(MQTT_CONNEECTED)
        {
            esp_mqtt_client_publish(client, MQTT_PUB_TEMP_SHT30 , kq, 14, 0, 0);
            //esp_mqtt_client_publish(client, MQTT_PUB_HUM_SHT30 , humi, 0, 0, 0);
            if(flag==0)
            {
                printf("%s",rain);
                esp_mqtt_client_publish(client, MQTT_PUB_ENV , rain, 9, 0, 0);
            }
            else{
                printf("%s",rain1);
                esp_mqtt_client_publish(client, MQTT_PUB_ENV , rain1, 6, 0, 0);
            }
            if(temperature > 35){
